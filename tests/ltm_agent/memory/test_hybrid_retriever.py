...
class TestHybridRetriever:
    ...
    async def test_search_with_vector_only(
        self, vector_store_mock, bm25_store_mock, sample_knowledge_units
    ):
        ...
        retriever = LangchainHybridRetriever(
            vector_store_mock, bm25_store_mock, vector_weight=1.0, bm25_weight=0.0
        )
    ...
    async def test_search_with_bm25_only(
        self, vector_store_mock, bm25_store_mock, sample_knowledge_units
    ):
        ...
        retriever = LangchainHybridRetriever(
            vector_store_mock, bm25_store_mock, vector_weight=0.0, bm25_weight=1.0
        )
    ...
    async def test_search_with_hybrid_weights(
        self, vector_store_mock, bm25_store_mock, sample_knowledge_units
    ):
        ...
        retriever = LangchainHybridRetriever(
            vector_store_mock, bm25_store_mock, vector_weight=0.4, bm25_weight=0.6
        )
    ...
    async def test_rerank_results(self, vector_store_mock, bm25_store_mock, sample_knowledge_units):
        ...
        # Create retriever
        retriever = LangchainHybridRetriever(
            vector_store_mock, bm25_store_mock, vector_weight=0.5, bm25_weight=0.5
        )
        ...
        long_unit = create_test_knowledge_unit(
            content="A" * 2000,
            context="Long content unit",  # Very long content
        )
