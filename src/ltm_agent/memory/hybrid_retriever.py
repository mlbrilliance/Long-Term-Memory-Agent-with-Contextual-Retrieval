...
import logging
import asyncio
from typing import List, Dict, Any, Optional, Tuple, Set, Counter, Union, cast
...
class LangchainHybridRetriever(HybridRetriever):
    ...
    async def search(
        ...
        if v_weight > 0:
            ...
            search_tasks.append(self.vector_store.similarity_search(
                embedding,
        ...
        if b_weight > 0:
            search_tasks.append(self.bm25_store.search(
                query,
    ...
    async def _combine_results(
        ...
        if vector_results:
            ...
            if vector_max_score > 0:
                vector_results = [(unit, score / vector_max_score) for unit, score in vector_results]
