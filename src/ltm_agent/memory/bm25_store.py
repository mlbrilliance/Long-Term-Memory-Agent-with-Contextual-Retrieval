...
try:
    ...
    raise ImportError(
        "The rank_bm25 package is required for BM25 search. "
...
class RankBM25Store(BaseBM25Store):
    ...
    async def remove(self, unit_ids: List[str]) -> None:
        ...
        logger.info(f"Removed {len(remove_ids.intersection(current_ids))} units from BM25 index. Remaining: {len(self.id_map)}")
